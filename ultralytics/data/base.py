    ...
    def load_image(self, i, rect_mode=True):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im_depth = cv2.imread(f.replace('rgb', 'depth'))  # BGR
                
                if im_rgb is None or im_depth is None:
                    raise FileNotFoundError(f"Image Not Found {f}")

                h_vis, w_vis = im_rgb.shape[:2]  # orig hw
                h_inf, w_inf = im_depth.shape[:2]  # orig hw
                ...
                b2, g2, r2 = cv2.split(im_depth)
               
                #im = cv2.merge((b, g, r, b2, g2, r2))
                im = cv2.merge((b, g, r, b2))
